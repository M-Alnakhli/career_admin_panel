...
if(data!==null){
    if(data?.status==='sucess'){
        ...
        authContext.login({role:data.role!==undefined?data.role:null,token:data.token!==undefined?data.token:null})
    }
    else{
    ...
    {({
      ...
      <form style={{flex:1,display:'flex',flexDirection:'column',justifyContent:'space-around'}} onSubmit={handleSubmit}>
       ...
       <div style={{flex:1,display:'flex',justifyContent:'space-between',marginBottom:'3px',width:'30ch'}}>
        ...
        {errors.email && touched.email && errors.email&& <Label style={{color:'red'}}> {errors.email && touched.email && errors.email}</Label> }
        ...
        />
        {errors.password && touched.password && errors.password&& <Label> {errors.password && touched.password && errors.password}</Label>}
        </div>
