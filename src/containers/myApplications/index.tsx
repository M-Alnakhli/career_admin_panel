...
Error
  ...
  return (
    ...
    <Card  style={{display:'flex',flex:1,flexDirection:'column',backgroundColor:'white',flexWrap: 'wrap',paddingLeft:'10px',paddingRight:'10px'}} >
      <Header style={{alignSelf:'flex-start',marginTop:'30px',marginBottom:'30px'}} size={30}>My Applicants</Header>
      <div  style={{display:'flex',flex:1,flexWrap: 'wrap'}}>
    {data!==null&&data.applications.map((element,index)=><MyApplicationlistRow key={index} applicationItem={element} refrech={()=>console.log('fdfkjdf')}/>)}
    </div>
    </Card>
    </div>
  )
