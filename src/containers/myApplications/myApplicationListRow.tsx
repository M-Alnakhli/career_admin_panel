...
export type Props  ={applicationItem:ApplicationListItem,refrech:()=>void}
export const MyApplicationlistRow = (props:Props) => {
    const navigation  =useNavigate()
    const [data,errors,loading,deleteApplication]= useDeleteApplicationAPI()
    const onClick=()=>{
        navigation(`/applicationForm/${props.applicationItem.applicationId}`,{state:{item:{...props}}})
    }
  ...
  return (
    <div className="listItem" >
        ...
        <Label style={{fontFamily:'Exo-Medium',fontSize:'small'}}>{props.applicationItem.posistion}</Label>
        ...
        <div  style={{position:'absolute',bottom:'0px',right:'5px',display:'flex'}}>
            ...
            <Edit style={{marginRight:'10px'}} width={'15px'} height={'15px'}/>
            <Trash onClick={()=>deleteApplication(props.applicationItem.applicationId as string)} width={'15px'} height={'15px'}/>
            </>:<Label style={{color:statusColorRendarer(props.applicationItem.status as ApplicationStatusType)}}>{props.applicationItem.status}</Label>}
        </div> 
        </div>
