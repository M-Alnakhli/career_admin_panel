...
import {
  CarearListAPIResType,
  CareerConfig,
  CareerListAPIReqType,
} from "../../api/typs";
...
export const Carears = () => {
  const [query, setQuery] = React.useState<CareerListAPIReqType>({
    offset: 0,
    pageNumber: 0,
    location: null,
    positionId: null,
    ...
    <div style={{ padding: "3ch", flex: 1 }}>
      ...
      >
        ...
        <div style={{ display: "flex", flex: 1, flexWrap: "wrap" }}>
          {data !== null &&
            data.carears.map(
              (element: CareerConfig, index: React.Key | null | undefined) => (
                <CarearlistRow key={index} {...element} />
              )
            )}
        </div>
      </Card>
