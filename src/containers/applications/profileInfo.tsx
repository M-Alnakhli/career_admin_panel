...
export const ProfileInfo = (props: Props) => {
  ...
  const [value, setValue] = React.useState<ApplicationStatusType>(
    props.applicationStatus
  );
  const [data, errors, loading] = useEvaluateApplicationAPI({
    applicant: props.applicantId,
    applicationStatus: props.applicationStatus,
  });
  const [statusData, statusError, statusLoading, updateApplicationStatus] =
    useUpdateStatusApplicationAPI({
      applicationId: props.applicantId,
      applicationStatus: props.applicationStatus as ApplicationStatusType,
    });
    ...
    return (
      <Label
        style={{
          ...
          fontSize: "small",
          color: statusColorRendarer(value),
        }}
      >
        {value}
      </Label>
  ...
  const renderSelcetion = () => {
    return (
      <Selction
        setValue={changeApllicationStatus as (val: string) => void}
        value={value as string}
        withHideValue={true}
        selctionId="applicationStatus"
        options={options}
        hideValueComponent={renderStatus}
        label={"status"}
      />
    );
  };
  ...
  return (
    <div style={{ marginLeft: "30px" }}>
      ...
      {tabel.map((element, index) => (
        <Row key={index} label={element.label} value={element.value} />
      ))}
    </div>
  );
};
