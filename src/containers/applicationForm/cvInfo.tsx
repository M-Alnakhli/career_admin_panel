...
<div style={{display:'flex',justifyContent:'space-between',marginTop:0}}>
     ...
     style={{height:'3vh'}}
        ...
        />
 <ErrorMessage name='linkedIn'>{(errors:string)=><Label style={{color:'red'}}>{errors}</Label>}</ErrorMessage>
        </div>
        ...
        <input
         style={{height:'3vh'}}
          ...
          value={values.cv}
        />
         <ErrorMessage name='cv'>{(errors:string)=><Label style={{color:'red'}}>{errors}</Label>}</ErrorMessage>
        </div>
